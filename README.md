...
- **`test-cases/<construct>/expected-entailments.ttl`** — the triples a conformant OWL 2 DL reasoner *must* derive from that premises file.
...
> All 27 active constructs are entailment-based (they must produce new, checkable triples). Eight further constructs test *violations* (an axiom that should make an ontology inconsistent, e.g. `owl:AsymmetricProperty`) — these currently live parked under `inconsistency-test-cases/`, not `test-cases/`, and aren't wired into any script for now. See that folder's README when you're ready to bring them back in.
...
python scripts/prepare_test_ontology.py [--target your-ontology.ttl]
                │
                ▼
      build/infertest-merged.ttl   <- every enabled construct's premises.ttl (+ your ontology)
                │
                ▼
     >>> run YOUR OWN reasoner over this file, however your pipeline already does that <<<
...
It checks entailment by **refutation** rather than by scraping reasoner output: to check whether `premises.ttl` entails triple `T`, it adds `T`'s logical negation (`owl:differentFrom` for an expected `owl:sameAs`, an `owl:NegativePropertyAssertion` for an expected property assertion, an `owl:complementOf` class for an expected `rdf:type`) and checks that the result is inconsistent — necessary because reasoner CLIs print summaries, not full closures (e.g. HermiT never prints inferred `owl:sameAs` merges at all). If the reasoner instead fails for an unrelated reason (a Java crash, a parsing error), that construct is reported as its own `ERROR` status rather than being silently counted as a pass or fail.
...
Exit `0` = all passed, `1` = something failed (CI-friendly) — but the failed construct(s) and their missing triples are always named, never just a bare pass/fail. `run_all_tests.py` can also report `ERROR` for a construct whose reasoner run didn't reach a definite answer at all (see above) — treat that as "investigate," not as a pass or a fail.
...
# range
# equivalent-classes
# ... (21 more, all commented out by default)
```
...
test-cases/<construct>/
    ...
    expected-entailments.ttl      Triples that must be entailed

inconsistency-test-cases/<construct>/
    (parked for now — violation-style constructs, not wired into any script)
...
2. Add `expected-entailments.ttl` with the triples that must be derivable.
